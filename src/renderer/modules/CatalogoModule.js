...
import { TesisModule } from "./TesisModule.js";
...
export class CatalogoModule {
  constructor() {
    ...
    this.tesisModule = new TesisModule();
    // Referencias DOM
    this.tabButtons = [];
    this.tabContents = [];
    ...
    this.moduleMap = {
      ...
      "btn-tab-tesis": () => this.tesisModule.init(),
    };
  }
  ...
  async initializeModuleByTab(buttonId) {
    ...
    if (this.initializedModules.has(buttonId)) {
      ...
      console.log(
        `💾 [CatalogoModule] Módulo "${buttonId}" ya cargado, usando caché`,
      );
      return;
    }
    ...
    if (initFn) {
      ...
      console.warn(
        `⚠️ [CatalogoModule] No hay módulo registrado para: ${buttonId}`,
      );
  ...
  setupTabNavigation() {
    ...
    container.addEventListener("scroll", () => this.updateTabArrows(), {
      passive: true,
    });
   ...
   */
  updateTabArrows() {
    if (!this.tabsContainer || !this.tabArrowLeft || !this.tabArrowRight)
      return;
