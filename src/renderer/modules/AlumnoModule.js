...
export class AlumnoModule {
  ...
  async init() {
    ...
    if (this.data.length > 0) {
      this.table = new DataTable({
        ...
        onEdit: 'alumnoModuleInstance.openEditModalFromMenu',
        onToggleStatus: 'alumnoModuleInstance.toggleAlumnoStatus',
      });
    ...
    // 🔹 DELEGACIÓN DE EVENTOS PARA MENÚ CONTEXTUAL
    if (tbody) {
  ...
  _openModal(alumno = null) {
    const modal = this.modalElement; const form = document.getElementById('form-alumno');
    if (!modal || !form) return;
    if (!alumno) { const hasAutosave = localStorage.getItem('autosave:alumno-form'); if (!hasAutosave) form.reset(); } 
    else { form.reset(); }
    const setVal = (id, val) => { const el = document.getElementById(id); if (el) el.value = val ?? ''; };
    setVal('alumno-id', ''); setVal('alumno-estado', 'activo');
    this.unsavedGuard = new UnsavedChangesGuard('#form-alumno');
    this.formAutosave = new FormAutosave('form-alumno', 'alumno-form');
    if (alumno) {
      setVal('alumno-id', alumno.id); setVal('alumno-matricula', alumno.matricula);
      setVal('alumno-ap-paterno', alumno.apellido_paterno); setVal('alumno-ap-materno', alumno.apellido_materno);
      setVal('alumno-nombres', alumno.nombres); setVal('alumno-correo', alumno.correo_contacto);
      setVal('alumno-telefono', alumno.telefono_contacto); setVal('alumno-programa', alumno.programa_academico);
      setVal('alumno-fecha', alumno.fecha_ingreso ? alumno.fecha_ingreso.split('T')[0] : '');
      setVal('alumno-tratamiento', alumno.tratamiento || 'Est.'); setVal('alumno-articulo', alumno.articulo || 'El');
  ...
  async _saveAlumno(modal) {
    ...
    const datos = {
      ...
      matricula: getVal('alumno-matricula'), apellido_paterno: getVal('alumno-ap-paterno'),
      apellido_materno: getVal('alumno-ap-materno'), nombres: getVal('alumno-nombres'),
      correo_contacto: getVal('alumno-correo'), telefono_contacto: getVal('alumno-telefono'),
    ...
    if (!datos.matricula || !datos.apellido_paterno || !datos.nombres || !datos.periodo_ingreso) {
      Toast.error('Campos obligatorios: Matrícula, Apellido Paterno, Nombres y Periodo de Ingreso', 7000); return;
    }
    const btnSave = document.getElementById('btn-guardar-alumno');
    ...
    try {
      ...
      } else { Toast.error(`Error: ${res.error || 'No se pudo guardar el alumno'}`, 7000); }
    } catch (error) {
      console.error('💥 Error guardando alumno:', error);
      Toast.error('Error de conexión: no se pudo guardar el alumno', 6000);
    } finally { if (btnSave) { btnSave.disabled = false; btnSave.innerHTML = originalText; } }
  }
  ...
  async toggleAlumnoStatus(rowId) {
    ...
    const alumno = this.data.find(a => 
      String(a.id) === String(rowId) || String(a.matricula) === String(rowId) || a.id == rowId || a.matricula == rowId
    );
    if (!alumno) { console.error('❌ Alumno no encontrado para rowId:', rowId); Toast.error('No se encontró el alumno', 5000); return; }
    ...
    const nombreCompleto = [alumno.nombres, alumno.apellido_paterno, alumno.apellido_materno].filter(p => p?.trim()).map(p => p.trim()).join(' ') || alumno.matricula;
    ...
    try {
      ...
      } else { Toast.error(`Error: ${res.error}`, 6000); }
    } catch (err) {
  ...
  openAssignmentModal(buttonEl) {
    ...
    if (!alumno) { console.error(`❌ Alumno no encontrado: ${alumnoId}`); return; }
    const nombreCompleto = [alumno.nombres, alumno.apellido_paterno, alumno.apellido_materno].filter(p => p?.trim()).map(p => p.trim()).join(' ');
    window.assignmentModal.open({ entityType: 'alumno', entityId: alumno.id, entityName: nombreCompleto, matricula: alumno.matricula });
  }
