...
export class DocenteModule {
  ...
  async init() {
    ...
    this.table = new DataTable({
      ...
      onEdit: "docenteModuleInstance.openEditModalFromMenu",
      onToggleStatus: "docenteModuleInstance.toggleDocenteStatus",
    });
    ...
    if (tbody) {
      tbody.addEventListener("click", (e) => {
        ...
        if (
          action === "toggle" &&
          typeof this.toggleDocenteStatus === "function"
        ) {
          this.toggleDocenteStatus(rowId);
        }
        // Para editar (cuando lo implementes en el futuro):
        else if (
          action === "edit" &&
  ...
  _setupModalEvents() {
    ...
    this.modalElement
      ...
      ?.addEventListener("click", intentarCerrar);
    document
      .getElementById("btn-cancelar-docente")
      ?.addEventListener("click", intentarCerrar);
  ...
  async toggleDocenteStatus(rowId) {
    ...
    // 🔹 Acciones con tiempos verbales correctos
    const accionInfinitivo =
    ...
    const confirmado = await globalConfirm.ask(
      `¿${accionInfinitivo} docente?`, // Título: "¿Activar docente?" o "¿Desactivar docente?"
      mensajeConfirmacion, // Mensaje: explicación detallada
    );
    ...
    // 🔹 Ejecutar actualización en backend
    try {
      ...
      const res = await window.electronAPI.actualizarEstadoDocente({
        id: docente.id,
        nuevoEstado,
      });

      if (res.success) {
        // 🔹 Toast en pasado con duración consistente (5s)
        Toast.success(
          `Docente ${accionPasado} correctamente: ${nombreCompleto}`,
          5000,
        );
  ...
  async _saveDocente(modal) {
    ...
    const esFemenino =
      tratamiento.endsWith("a.") || tratamientosFemeninos.includes(tratamiento);
    ...
    const datos = {
      ...
      apellido_paterno: document
        .getElementById("doc-ap-paterno")
        ?.value?.trim(),
      apellido_materno: document
        .getElementById("doc-ap-materno")
        ?.value?.trim(),
    ...
    if (!datos.codigo || !datos.apellido_paterno || !datos.nombres) {
      Toast.error(
        "Campos obligatorios: Código, Apellido Paterno y Nombres",
        6000,
      );
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        ...
        Toast.success(
          `Docente ${accion} correctamente: ${datos.nombres} ${datos.apellido_paterno}`,
          5000,
        );
        ...
        // ❌ ERROR DEL BACKEND
        Toast.error(
          `Error: ${res.error || "No se pudo guardar el docente"}`,
          7000,
        );
      }
    } catch (error) {
  ...
  loadRowSummary(event) {
    ...
    try {
      ...
      chips.innerHTML =
        '<span class="chip" style="color:var(--danger-color)">Error</span>';
    }
  }

  openAssignmentModal(buttonEl) {
    const row =
      buttonEl?.closest(".sub-row-details")?.previousElementSibling ||
